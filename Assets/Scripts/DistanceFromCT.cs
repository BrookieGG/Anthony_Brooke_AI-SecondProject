...
namespace NodeCanvas.Tasks.Conditions
{
    public class DistanceFromCT : ConditionTask
    {
        ...
        protected override bool OnCheck()
        {
            ...
            Vector3 eyePos = agent.transform.position;
            if (eye.value != null)
                eyePos = eye.value.position;

            Vector3 toTarget = target.value.position - eyePos;
            ...
            if (rangeMode == RangeMode.OutOfLineOfSight)
            {
                ...
                Vector3 rayDirection = (target.value.position - eyePos).normalized;
                if (Physics.Raycast(eyePos, rayDirection, toTarget.magnitude, mask.value, QueryTriggerInteraction.Ignore))
                    return true;
